...
uint16_t reading[BUFFER_SIZE]; //Sampling buffer
...
void setup(){ 
pinMode(SlowOut, OUTPUT);
pinMode(MiddleOut, OUTPUT);
pinMode(FastOut, OUTPUT);
...
hrate=hrate*60;
    ...
    MiddleOut = LOW;
    SlowOut = LOW;}
else if (hrate < 110 && hrate > 70)
{FastOut = LOW; MiddleOut = HIGH; SlowOut = LOW;}
else if (hrate < 70 && hrate > 60)
{FastOut = LOW; MiddleOut = LOW; SlowOut = HIGH;}
else {FastOut = LOW; MiddleOut = LOW; SlowOut = LOW;}
}
